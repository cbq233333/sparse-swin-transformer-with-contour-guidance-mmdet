...
class FCOSHeadEdge(AnchorFreeHeadEdge):
    ...
    def loss(self,
        ...
        for gt_mask in gt_masks:
            _,h_ori,w_ori = gt_mask.masks.shape
            gt_bitimg = numpy.zeros((h_ori,w_ori),dtype=numpy.float32)
            ...
            gt_bitimg=numpy.pad(gt_bitimg, ((0,h-h_ori),(0,w-w_ori)), 'constant')
            gt_edge = self.groundtruth_edge(gt_bitimg)
            #cv2.imwrite("/home/cbq/mmdetection/gt_bitimg_ssdd.jpg",gt_bitimg*255)
            #cv2.imwrite("/home/cbq/mmdetection/gt_edge_ssdd.jpg",gt_edge*255)
            gt_bitimg_t = torch.from_numpy(gt_bitimg)
            gt_edge_t = torch.from_numpy(gt_edge)
...
#     def get_edge(img,gt_masks):
#         #img:(H,W,C)
#         #gt_bbox:[x1,y1,x2,y2],(num,4)
#         img = img.permute(1, 2, 0).numpy()
#     # RGB转BRG
# #img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
#         bboxes = result['gt_bboxes'].data
#         for bbox in bboxes:
#             x1 = int(bbox[0])
#             y1 = int(bbox[1])
...
#             img_e[y1:y2,x1:x2,:] = 0
#         img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
#         cv2.imwrite('/workspace/mmdetection/img.jpg',img)
#         edges = result['gt_masks'].data.masks
#         for edge in edges:
#             length = len(edge[0])
#             xs = edge[0][0:length-1:2]
#             ys = edge[0][1:length:2] 
#             for k in range(len(ys)):
#                 img_e[int(ys[k]),int(xs[k]),:] = 255
#         cv2.imwrite('/workspace/mmdetection/img_edge.jpg',img_e)
...
#         #     img = cv2.imread(img_path)
#         #     blurred = cv2.GaussianBlur(img[y1:y2,x1:x2,:],(11,11),0)
#         #     gaussImg = cv2.Canny(blurred, 10, 70)
#         #     img_e = img
#         #     img_e[y1:y2,x1:x2,:] = 0
#         #     img_e[y1:y2,x1:x2,:] += gaussImg[:,:,None]
#         #     img_bbox = img[x1:x2,y1:y2]
